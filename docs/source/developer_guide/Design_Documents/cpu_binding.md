...
| Logical NPU map | `npu-smi info -m` | Maps card/chip IDs to global logical NPU IDs and gives `total_logic_npus`. On Ascend 950PR&950DT products, `Chip Logic ID` is not reported, so `NPU ID` is used as the logical ID. |
| Running NPUs | `npu-smi info` process table, filtered by `ASCEND_RT_VISIBLE_DEVICES` | Identifies the logical NPUs used by this worker process. A2/A3 process rows use `NPU Chip`; Ascend 950PR&950DT products process rows use `NPU ID`. |
...
| Thread topology | `lscpu` `Thread(s) per core` | Determines Ascend 950PR&950DT products cluster size: 8 CPUs for 1 thread per core, 16 CPUs for 2 threads per core. |
| UVB polling threads | `ps -Te` | Finds host `uvb_poll_window_thread` threads for Ascend 950PR&950DT products UVB CPU binding. Docker containers must use `--pid=host` to see these host threads. |
...
| Ascend 950PR&950DT products | `topo_affinity` | Ascend 950PR&950DT products use NPU-to-CPU affinity from `npu-smi info -t topo` to choose an affinity NUMA node, then assign one CPU cluster from that NUMA node to each worker. They also report process rows by `NPU ID` instead of `NPU Chip`, skip IRQ binding, and bind host UVB polling threads. |
...
`topo_affinity` is designed for A2, Atlas 300 inference products, Ascend 950PR&950DT products,
...
For Ascend 950PR&950DT products, topology affinity is used differently:
...
If Ascend 950PR&950DT products topo affinity is missing, spans multiple NUMA nodes, has too few
...
is 5 CPUs per NPU for devices with IRQ binding. Ascend 950PR&950DT products require one full
cluster per worker.
...
- IRQ binding places NPU IRQ handling on the CPUs reserved for the corresponding
  ...
  Ascend 950PR&950DT products skip this step.
...
Ascend 950PR&950DT products use a different role split, so their plan log does not include ACL or
...
[cpu_bind_mode] mode=topo_affinity rank=0 visible_npus=[0]
The CPU allocation plan is as follows:
Ascend 950PR&950DT products NPU0: worker=[...]
...
- Each final NPU pool must have enough CPUs for its role split: at least 5 CPUs
  for devices with IRQ binding. Ascend 950PR&950DT products require one complete CPU cluster per worker.
...
- IRQ binding requires writable `/proc/irq` and resolvable PCI/IRQ information.
  Ascend 950PR&950DT products skip IRQ binding even when `/proc/irq` is writable.
- Ascend 950PR&950DT products UVB polling thread binding requires visibility into the host PID
