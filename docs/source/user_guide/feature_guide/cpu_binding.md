...
On Ascend 950PR&950DT products, CPU binding uses NPU-to-CPU affinity from `npu-smi info -t topo`
...
`Thread(s) per core`: 8 CPUs when it is 1, and 16 CPUs when it is 2. Ascend 950PR&950DT products
also pin host `uvb_poll_window_thread` threads to NUMA0 CPUs except CPU0,
...
Ascend 950PR&950DT products still can migrate memory pages when `migratepages` is available, but
they do not separately pin ACL/release threads and do not apply IRQ binding.
...
Ascend 950PR&950DT products do not apply IRQ binding. When running on Ascend 950PR&950DT products, the log contains
`[irq] IRQ binding skipped on Ascend 950PR&950DT products.` and no `/proc/irq/*/smp_affinity` files are
written by this feature.

Ascend 950PR&950DT products allocation logs use `worker=[...]` instead of `acl=[...]` or
...
| `Insufficient CPUs for binding...` | Fewer CPUs are available than the role split requires. Devices with IRQ binding need at least 5 CPUs per logical NPU. Ascend 950PR&950DT products need one full cluster per worker. | Expand the cpuset or reduce visible NPUs. |
| `NPU topo affinity not found...` | Topology affinity is unavailable. | On Ascend 950PR&950DT products, worker CPU binding is skipped. On other topo-affinity devices, vLLM Ascend falls back to `global_slice`. Check `npu-smi info -t topo` when topology affinity is expected. |
| `uvb_poll_window_thread not found... --pid=host` | Ascend 950PR&950DT products could not see host UVB polling threads. | Recreate the Docker container with `--pid=host`, then restart vLLM. |
| `failed to bind uvb_poll_window_thread... --pid=host` | Ascend 950PR&950DT products found a UVB polling thread but failed to bind it. | Check permissions and recreate the Docker container with `--pid=host` if running in Docker. |
...
| `[irq] IRQ binding skipped on Ascend 950PR&950DT products.` | Ascend 950PR&950DT products do not use the IRQ binding step. | No action needed. Worker main binding and memory migration still proceed. |
