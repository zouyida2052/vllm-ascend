    ...
    def _handle_request(  # type: ignore[override]
        ...
        if isinstance(request, _LayerRevokeTask):
            try:
                ...
                self.request_queue.task_done()
            return
        if request and request[0].use_key_major_ranges:
            # The range handler owns queue completion, including failures.
            self._handle_range_layer_tasks(request)
            return
        try:
            self._handle_layer_transfer(request)
        finally:
            self.request_queue.task_done()
